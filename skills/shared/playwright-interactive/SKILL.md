...
- **Existing user Chrome session**: Prefer the shared `playwriter` MCP when the user needs the agent to work in their normal Chrome profile, reuse existing auth/extensions, bypass automation-specific login failures, or collaborate on a tab the user can see and control.
- **Cursor / Codex**: Use the `js_repl` Playwright workflow below (this file’s primary instructions).
- **OpenCode**: Prefer `agent-browser` with `--headed` and `--json`; load the browser skill first; reuse one session across iterations. When the active model cannot view images, collect HTML/DOM/a11y evidence alongside screenshots.
...
## Playwriter MCP For Real Chrome

Use this path before launching a separate browser when the task depends on the user's real Chrome session, especially Figma, Google login, password-manager flows, installed extensions, SSO, captchas, or reproducing a bug in an already-open tab.

Requirements:

- Shared MCP config exists at `mcp/shared/playwriter.json` and has been synced into the active agent/tool config.
- The user has installed the Playwriter Chrome extension from the Chrome Web Store.
- The user has opened the target tab in normal Chrome and clicked the Playwriter extension icon for that tab.
- The Playwriter icon is green before the agent tries to control the tab.

Workflow:

1. Ask the user to open the target page in their normal Chrome profile if it is not already open.
2. Ask the user to click the Playwriter extension icon on that tab and confirm it turned green.
3. Use the `playwriter` MCP `execute` tool to run small Playwright snippets against the connected browser.
4. Keep snippets short and observable: list pages, get URL/title, take a screenshot, inspect DOM/a11y text, then interact.
5. If auth or anti-bot flow blocks automation, ask the user to disconnect the extension, complete the manual step, then reconnect it.

Do not use Playwriter for unattended CI-style checks, broad crawls, or pages the user has not explicitly connected. It can only control tabs the user enabled; that consent boundary is the whole point, not an annoying implementation detail.
