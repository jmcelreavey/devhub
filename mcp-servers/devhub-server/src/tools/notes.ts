...
export function registerNotesTools(server: McpServer, ctx: Context): void {
  ...
  server.registerTool(
    ...
    {
      description:
        "Open an existing note in a new workspace tab in the running DevHub desktop app. Fails cleanly when the desktop app is not connected.",
      inputSchema: {
    ...
    async ({ path: notePath }) => {
      ...
      return withDashboardErrors(async () => {
        ...
        return { content: [{ type: "text", text: `Opened ${id} in a new DevHub desktop tab.` }] };
      });
    },
  );
