...
| Open          | `POST /api/repos/<name>/open`           | Cursor CLI when available. Optional `{ notePath }` opens a linked note as a persistent Markdown working copy alongside the repo (see [Cursor note working copies](../architecture/notes-system.md#cursor-note-working-copies)). |
