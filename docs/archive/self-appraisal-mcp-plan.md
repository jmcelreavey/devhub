...
those moments _as they happen_ and have them land in a tidy, factual, per-year
...
- No automated ingestion of others' feedback (manager/peer/360) — appraiser
  notes are things _you_ record about someone, not a feedback inbox.
...
`mcp-servers/devhub-server/src/tools/`, alongside `notes_*`, `docs_*`, `tasks_*`,
...
Appraiser notes are about _other people_, so they're more sensitive than
...
1. Add the tools to `mcp-servers/devhub-server/src/tools/appraisal.ts`: `appraisal_record`,
