...
| `devhub-create-tasks-from` | Note **Spawn tasks from plan** | Plan note → gist + Jira sub-tasks + DevHub tasks with entity links. Does not implement. See [Notes System](../architecture/notes-system.md#create-tasks-from-plan). |
| `devhub-implement-task` | Task **Implement with Agent…** | Implement one DevHub task end-to-end. Asks before commit/PR/Jira/complete. See [Dashboard](../architecture/dashboard.md#implement-with-agent). |
| `devhub-debug-hang` | Status hang, MCP `status_exec` | Diagnose a wedged dashboard. Start with in-flight external commands, not a restart. See [Dashboard — When the dashboard hangs](../architecture/dashboard.md#when-the-dashboard-hangs). |
