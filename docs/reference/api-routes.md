...
| `GET/POST/PATCH/DELETE /api/briefing/tasks` | Briefing **Research** drawer | CRUD for background research tasks. `POST` body `{ topic }` queues a Last30Days dig (or AI brief fallback). Task state in `notes/.cache/briefing/tasks.json`; results under `LAST30DAYS_MEMORY_DIR`. |
...
| `GET /api/appraisal/evidence?days=` | `/appraisal` evidence panel | Suggests recordable moments from recent PRs, Jira, and tasks (default window from `EVIDENCE_RANGE_DAYS` presets). |
