...
| `npm run desktop:build`  | Stage, build, and ad-hoc sign a release bundle       |
...
| `bash scripts/devhub-backport.sh <source-ref> [--base <ref>] [--execute] [--title "…"] [--patch-only] [--require-synced]` | Build a clean PR branch off `upstream` and port the feature's hunks onto it (`git apply --3way`, preserving public-side templatisation). Backport is **allowlist-based** (`PUBLIC_PATHS` in `scripts/lib/public-paths.sh`): `agents/shared`, `dashboard`, `desktop`, `docs`, `mcp/shared`, `mcp-servers`, `opencode/shared`, `persona/` (shared files only), `scripts`, `shared`, `skills/shared`, and root catalog files — not a denylist of personal paths. Dropped automatically: `notes/`, `tasks/`, `collections/`, `upstarts/`, `persona/identity.txt`, and any `skills/*` outside `skills/shared/`. Added lines are leak-scanned. `--base` defaults to `<source-ref>^` (pass the branch point for multi-commit features). `--patch-only` prints the leak-scanned patch to stdout and exits `3` when nothing public remains (used by `devhub-ship.sh --dry-run`). `--require-synced` fails unless `refs/devhub/upstream-sync` matches `upstream/HEAD` (ship uses this before pushing public core). Previews by default; `--execute` pushes the branch to `upstream` and opens the PR. |
| `bash scripts/devhub-ship.sh ["commit message"] [--dry-run] [--no-upstream]`            | One-shot **ship everything**: commit personal paths separately, import newer public-core changes, commit remaining work, push `origin main` (pre-push verify runs), preview and port the generic catalog patch onto `upstream` main directly (leak-scanned, no PR), then commit+push every enabled plugin repo. `--dry-run` prints the actual public patch, including committed divergence. Requires `main`/`master`. `--no-upstream` skips reconciliation and the public-core push. MCP `repo_ship` previews unless `confirm: true`. On success, prints a reminder to **Rebuild Dashboard** or **Attach to Dev Server** so the installed `.app` picks up dashboard changes. |
