...
import { DbRefusedError, executeStatement, planExecution } from "@/lib/db/execute";
...
const QuerySchema = z.object({
  statement: z.string().trim().min(1, "statement is required").max(200_000, "statement too long"),
  rowLimit: z.number().int().positive().max(DB_MAX_ROW_LIMIT).optional(),
  timeoutMs: z.number().int().positive().max(DB_QUERY_MAX_TIMEOUT_MS).optional(),
