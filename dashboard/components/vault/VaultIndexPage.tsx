...
function IndexFileRow({ file }: { file: VaultIndexFile }) {
  ...
  return (
    <Link
      href={file.href}
      className="group flex items-center gap-2 rounded-md px-2 py-1.5 text-sm no-underline transition-colors hover:bg-[var(--bg-elevated)] text-text"
    >
      <FileText
        size={14}
      ...
      <ChevronRight
        size={12}
        className="shrink-0 reveal-on-hover transition-opacity text-text-subtle"
        aria-hidden
      />
    </Link>
  );
    ...
    <div className="page-wrapper max-w-3xl">
      <header className="page-header mb-6" style={{ alignItems: "flex-start" }}>
        <div className="min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-lg font-semibold m-0 text-text">
              {title}
            </h1>
          </div>
          ...
          {footerHint ? (
            <p className="text-xs mt-2 m-0 text-text-subtle">
              {footerHint}
            </p>
          ) : null}
        </div>
      ...
      {summary.totalFiles === 0 ? (
        ...
        <div className="space-y-6">
          {summary.recent.length > 0 ? (
            <section className="card card-body">
              <div className="flex items-center gap-2 mb-3">
                <Clock size={14} className="text-text-subtle" aria-hidden />
                <h2 className="text-sm font-semibold m-0 text-text">
                  Recent
                </h2>
              </div>
          ...
          <section className="card card-body">
            <h2 className="text-sm font-semibold m-0 mb-3 text-text">
              Contents
            </h2>
            {summary.rootFiles.length > 0 ? (
              <div className="mb-2">
                <h3
                  className="mb-1 px-2 text-xs font-semibold tracking-tight text-text-subtle"
                >
                  Top level
                </h3>
