...
import { NotesViewToggle, type NotesPanelView } from "@/components/NotesViewToggle";
import { ResizeHandle } from "@/components/ResizeHandle";
import { createPersistedBoolStore } from "@/lib/use-persisted-bool";
import { useIsMobile } from "@/lib/use-is-mobile";

const STORAGE_KEY = "notes-files-sidebar-collapsed";
const WIDTH_STORAGE_KEY = "notes-files-sidebar-width";
const STORAGE_EVENT = "devhub:notes-files-sidebar-storage";
const usePersistedBool = createPersistedBoolStore(STORAGE_EVENT);
...
function subscribeStoredWidth(callback: () => void): () => void {
...
export function NotesFilesSidebar({
  panel,
  onPanelChange,
  search,
  onSearch,
      ...
      {collapsed ? (
        ...
        <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
          ...
          {panel === "files" ? (
            ...
            <p className="px-3 py-2 text-xs leading-relaxed" style={{ color: "var(--text-subtle)" }}>
              Checklists open in the main area. Switch to Files to browse notes.
            </p>
          )}
        </div>
      )}
  ...
  return (
    <div className="shrink-0 border-b" style={{ borderColor: "var(--border)" }}>
      <div className="px-2 pt-2 pb-2">
        <NotesViewToggle value={panel} onChange={onPanelChange} />
      </div>
      {showFileSearch ? (
        <>
