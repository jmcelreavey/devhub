...
export function PWAInstallPrompt() {
  ...
  return (
    <div
      className="card"
      /*
        Was bottom-centred, which put a 30rem card straight over the middle of
        whatever you were reading — on Today it covered two Jira rows. Bottom
        *right* keeps it out of the content column and matches where this app
        already puts transient chrome (see .toast-stack). It sits one layer
        above the toasts because it persists until dismissed while they expire;
        if both are up at once the prompt wins, which is the right call for the
        one you have to act on.
      */
      style={{
        position: "fixed",
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
        ...
        <span className="text-sm flex-1 min-w-0 text-text">
      ...
      {installed ? (
        ...
        <span className="text-xs leading-snug text-text-muted">
          ...
          <strong className="font-medium text-text">Install</strong> or{" "}
          <strong className="font-medium text-text">Uninstall</strong> DevHub, or reload and try again.
        </span>
      )}
      {installed && showUninstallHelp && (
        <div className="w-full space-y-2 text-xs text-text-muted">
          ...
          {!runningStandalone && typeof navigator !== "undefined" && /Chrome|Chromium|Edg\//i.test(navigator.userAgent) && (
            <p>
              In this normal tab, Chrome/Edge may show <strong className="font-medium text-text">Open in app</strong> in the address bar - that means DevHub is installed. Open it from there to get the app window, then use that window’s menu to uninstall.
            </p>
          )}
          {!runningStandalone && (
            <p>
              <button
                type="button"
                className="underline decoration-dotted underline-offset-2 hover:decoration-solid text-text-subtle"
                onClick={() => {
