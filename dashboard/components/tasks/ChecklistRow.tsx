...
function RowIconButton({
  ...
  return (
    <button
      ...
      className="flex h-6 w-6 shrink-0 items-center justify-center rounded border-0 bg-transparent p-0 transition-colors hover:bg-[var(--bg-overlay)] text-text-muted"
      onClick={onClick}
      aria-label={label}
    >
      {children}
      ...
      {!linked && onPromote ? (
        <HoverTip label={masterName ? `Link to ${masterName}` : "Link to master"}>
          <RowIconButton label={`Link ${label} to master`} onClick={onPromote}>
            <Link size={14} strokeWidth={2} className="text-accent" aria-hidden />
          </RowIconButton>
        </HoverTip>
      ) : null}
      {onDelete ? (
