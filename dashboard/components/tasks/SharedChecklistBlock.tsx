...
import { ChecklistRow } from "@/components/tasks/ChecklistRow";
...
import { useNoteEditorPath } from "@/lib/notes/editor-context";
import {
  ...
  return (
    ...
    >
      ...
      {master ? (
        ...
        >
          <ChecklistIcon name={master.icon} size={12} className="text-accent" />
      ...
      {!master && !isLoading ? (
        <div
          className="mb-2 flex items-center justify-between gap-3 text-xs text-text-subtle"
        >
          ...
          >
            {pickerOptions.map((item) => (
              <li key={item.id}>
                ...
                >
                  <LinkIcon size={12} className="text-accent" aria-hidden />
