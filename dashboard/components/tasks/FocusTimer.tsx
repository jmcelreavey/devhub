...
export function FocusTimer() {
  ...
  // Shared 1 Hz heartbeat (lib/tickers.ts), subscribed only while a session is
  // running. 0 before the store seeds, which the readout treats as "no time yet"
  // exactly as the previous `null` did.
  const tick = useSecondTick(!!session);
  ...
  /** Reset the once-only expiry latch whenever a new session starts. */
  useEffect(() => {
   ...
   *
   * Previously this rode inside the component's own `setInterval` tick,
   * specifically to avoid an effect that depends on `now` and re-runs every
   * second. With the shared ticker the dependency is unavoidable, but the
   * `expiredRef` latch already made the body idempotent — it fires exactly
   * once per session regardless of how often the effect re-runs.
   */
  useEffect(() => {
    if (!session || now === null) return;
    if (expiredRef.current || now < session.endsAt) return;
  ...
  const start = useCallback((mins: number) => {
    ...
    // No need to prime a local clock: writing the session flips the ticker's
    // `active` flag, and it seeds its timestamp on the first read.
  ...
  const stop = useCallback(() => {
    // Clearing the session unsubscribes the ticker, which drops `now` back to
      ...
      >
        ...
        >
          <div className="flex items-center justify-between mb-3">
            <h3 id="focus-custom-modal-title" className="text-sm font-semibold text-text">
              Custom duration
            </h3>
