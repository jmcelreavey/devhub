          ...
          <div key={file.path} className="card overflow-hidden" style={{ padding: 0 }}>
            ...
            >
              <div className="flex items-start gap-2 min-w-0 flex-1">
                ...
                <div className="min-w-0">
                  <Link href={href} className="text-sm font-medium break-words leading-snug hover:underline text-text">
                    {displayPath}
                  </Link>
            ...
            <div className="px-3.5 py-2">
              {file.matches.slice(0, 5).map((match, i) => (
                ...
                >
                  <span className="text-[11px] font-mono w-8 text-right shrink-0 select-none text-text-muted">
                    L{match.line}
                  </span>
                  <span className="text-xs leading-relaxed flex-1 text-text-subtle">
                    {semantic ? match.text : highlightSearchText(match.text, query)}
                  </span>
                </div>
              ))}
              {file.matches.length > 5 ? (
                <Link href={href} className="text-xs mt-1 inline-block text-accent">
