...
function renderInline(text: string, fileBase?: string): ReactNode[] {
  ...
  while (remaining) {
    ...
    if (match === boldMatch) {
      parts.push(
        <strong key={key++} className="text-text">
          {match[1]}
        </strong>,
      );
...
function CodeBlock({ code, lang }: { code: string; lang: string }) {
  ...
  return (
    <div className="my-2 rounded overflow-hidden" style={{ border: "1px solid var(--border)" }}>
      ...
      >
        <span className="text-[10px] font-medium tracking-tight text-text-subtle">
          {lang || "code"}
        </span>
        <button
          type="button"
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith("### ")) {
      out.push(
        <h3
          key={key++}
          className={`${compact ? "text-xs font-semibold mb-0.5" : "text-xs font-semibold mt-3 mb-1"} text-text`}
        >
      ...
      out.push(
        <h2
          key={key++}
          className={`${compact ? "text-xs font-semibold mb-0.5" : "text-sm font-semibold mt-4 mb-1"} text-text`}
        >
      ...
      out.push(
        <h1
          key={key++}
          className={`${compact ? "text-xs font-semibold mb-0.5" : "text-[15px] font-bold mt-2 mb-1"} text-text`}
        >
      ...
      out.push(
        <div key={key++} className="text-xs leading-relaxed pl-3 text-text-muted">
      ...
      out.push(
        <div key={key++} className="text-xs leading-relaxed pl-3 text-text-muted">
          <span className="text-text-subtle">{m[1]}.</span> {renderInline(m[2], fileBase)}
        </div>,
      );
    } else if (line.trim() === "") {
