...
export function RepoBranchPanel({ repoName, repoPath, onMutate }: RepoBranchPanelProps) {
  ...
  return (
    <div ref={panelRef} className="relative inline-flex">
      ...
      >
        {loading && !data ? (
          <div className="p-3 text-xs text-text-muted">Loading branches...</div>
        ) : !data ? (
          <div className="p-3 text-xs text-danger">Failed to load branches</div>
        ) : (
          <div>
              ...
              {data.branches.map((b) => (
                ...
                >
                  {b.current ? (
                    ...
                    <CornerDownLeft size={12} className="text-text-subtle" />
                  )}
                  <span style={{ fontWeight: b.current ? 600 : 400 }}>{b.name}</span>
                  {b.current && (
                    <span className="text-xs ml-auto text-accent">current</span>
                  )}
                </button>
              ))}
            </div>
