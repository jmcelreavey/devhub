      ...
      {open && (
        ...
        >
          {loadingDetails && !details && (
            <div className="p-2 text-xs text-text-subtle">
              Loading git details...
            </div>
          )}
          {hasDirty && (
                ...
                <div className="mb-2 rounded border p-2" style={{ borderColor: "var(--border)" }}>
                  ...
                  <div className="space-y-1">
                    {details.changedFiles.map((file) => (
                      <div key={`${file.status}:${file.path}`} className="flex gap-2 text-xs font-mono text-text-muted">
                    ...
                    {details.unpushedCommits.map((commit) => (
                      <details key={commit.hash} className="text-xs">
                        <summary className="cursor-pointer text-text">
                          <span className="font-mono text-accent">{commit.hash}</span> {commit.subject}
                        </summary>
                        {commit.files.length > 0 && (
                          <div className="mt-1 space-y-0.5 pl-4 font-mono text-text-muted">
