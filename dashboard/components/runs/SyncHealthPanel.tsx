    ...
    <div className="flex flex-col gap-4">
      ...
      >
        <div className="flex items-center gap-2 text-sm">
          {data.healthy ? <Check size={14} className="text-success" /> : <AlertTriangle size={14} className="text-warning" />}
      ...
      {!data.healthy && (
        ...
        >
          {data.missing.slice(0, 8).map((m) => (
            <div key={`${m.tool}-${m.name}`}><span className="text-warning">MISSING</span> [{m.tool}] {m.name}</div>
          ))}
        </div>
      )}
