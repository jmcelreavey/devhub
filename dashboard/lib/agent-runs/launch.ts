 ...
 */
export function agentRunnerCommand(runDir: string): string {
  const serverDir = process.env.DEVHUB_SERVER_DIR?.trim();
  if (serverDir) {
    const bundled = path.resolve(serverDir, "..", "services", "agent-run.cjs");
    if (fs.existsSync(bundled)) return [process.execPath, bundled, runDir].map(shellQuote).join(" ");
  }
  ...
  for (const dir of dashboardDirs) {
    ...
    if (fs.existsSync(tsx) && fs.existsSync(script)) return [tsx, script, runDir].map(shellQuote).join(" ");
  }
  throw new Error(
