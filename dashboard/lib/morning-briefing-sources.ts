...
// live in ./morning-briefing.
//
// All functions are parameterised by BriefingPrefs so the same code serves any
// user's location, feeds, and languages. The defaults live in briefing-prefs.ts.
...
export async function fetchTrendingRepos(languages: string[], limit = 20): Promise<RepoItem[]> {
  ...
  const results = await Promise.all(
    languages.map((lang) => {
      const q = encodeURIComponent(`created:>${since} language:${lang}`);
      const url = `https://api.github.com/search/repositories?q=${q}&sort=stars&order=desc&per_page=${limit}`;
      return fetchJson<GithubSearchResponse>(url, headers);
    }),
  );
  ...
  for (const res of results) {
    for (const item of res?.items ?? []) {
      if (!item.full_name || !item.html_url || seen.has(item.full_name)) continue;
